...
>
    ...
    <!--
     ...
     note:		if @print="no", ignore it.
                @chunk and @copy-to processing is try and error development.
                The information on processing requirement is not sufficient.
                Following code only covers the examples in the DITA specification:
                2.1.3.7 Chunking
                http://docs.oasis-open.org/dita/v1.2/os/spec/archSpec/chunking.html#chunking
                2015-09-10 t.makita
     -->
    <xsl:template match="*[contains(@class,' map/topicref ')][string(@print) eq 'no']" priority="8">
    ...
    <!-- Normal pattern: copy self and process descendant -->
    ...
    <!-- copy topicref itself and process only @chunk="to-content" topicref in the descendant -->
    <xsl:template name="copyWoDescendantAndProcessToContent">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:apply-templates select="* except *[contains(@class,' map/topicref ')]"/>
        </xsl:copy>
        <xsl:apply-templates select="descendant::*[contains(@class,' map/topicref ')][ahf:HasAttr(@chunk,'to-content')][1]"/>
    </xsl:template>

    <!-- @chunk="to-content" & no @copy-to-->
    <xsl:template match="*[contains(@class,' map/topicref ')][ahf:HasAttr(@chunk,'to-content')][empty(@copy-to)]" priority="6">
        <xsl:call-template name="copyWoDescendantAndProcessToContent"/>
    </xsl:template>

    <!-- @chunk="to-content" & "select-branch" & @copy-to-->
    <xsl:template match="*[contains(@class,' map/topicref ')][ahf:HasAttr(@chunk,'to-content')][ahf:HasAttr(@chunk,'select-branch')][exists(@copy-to)]" priority="6">
        <xsl:call-template name="copyWoDescendantAndProcessToContent"/>
    </xsl:template>
    
    <!-- @chunk="to-content" & specified @copy-to and parent map contains @chunk="by-document"-->
    <xsl:template match="*[contains(@class,' map/topicref ')][ahf:HasAttr(@chunk,'to-content')][exists(@copy-to)][ahf:HasAttr(parent::*[contains(@class,' map/map ')]/@chunk,'by-document')]" priority="6">
        <xsl:call-template name="copyWoDescendantAndProcessToContent"/>
    </xsl:template>

    <!-- @chunk="to-content" is specified in map -->
    <xsl:template match="*[contains(@class,' map/topicref ')][ahf:HasAttr(parent::*[contains(@class,' map/map ')]/@chunk,'to-content')]" priority="4">
        <xsl:call-template name="copyWoDescendantAndProcessToContent"/>
    </xsl:template>
